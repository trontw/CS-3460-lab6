...
public class BST {
	...
	private Node delete(Node root2, int time) {
		...
		if (time < root2.getTime()) {
			...
			root2.setLeft(delete(root2.getLeft(), time));
			...
			//*** Case 2: One child (right child present) - return y ***
			...
			Node temp = min(root2.getRight());
			//System.out.println("temp time is = "+temp.getTime());
			//System.out.println("root2 time is = "+root2.getTime());
			//Copying y's data onto root2
			root2.setTime(temp.getTime());
			root2.setReq_index(temp.getReq_index());
			...
			////root2.setLeft(null);
			root2.setRight(delete(root2.getRight(), temp.getTime()));
			return root2;
		}
		return root2;
	}
